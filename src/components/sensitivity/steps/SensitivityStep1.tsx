...
import {SensitivityData , SensitivityState} from "@/types/sensitivity";
import {ReactionLevel} from "@/types/common";

interface SensitivityCardData {
    element: keyof SensitivityState;
    title: string;
    description: string;
    emoji: string;
    options: {
...
interface Props {
    onNext: (data: SensitivityData) => void;
    onPrev: () => void;
}
...
export default function SensitivityStep1({ onNext, onPrev }: Props) {
    ...
    const handleOptionSelect = (element: keyof SensitivityState, value: ReactionLevel) => {
        setSensitivity(prev => ({
            ...prev,
            [element]: value
        }));
            ...
            <div className="container mx-auto px-6 py-8">
                ...
                <div className="mb-8">
                    ...
                    <div className="w-full bg-white/20 rounded-full h-1.5">
                        <div className="bg-white rounded-full h-1.5 w-1/3 transition-all duration-300"></div>
                    </div>
                </div>
                ...
                <Card variant="elevated" padding="lg" className="mb-6 text-gray-900">
                    <CardHeader className="text-center">
                        ...
                        <p className="text-gray-600 text-sm">각 요소별로 얼마나 민감하신지 선택해주세요</p>
                    </CardHeader>
                    ...
                    <CardContent>
                        ...
                        {isComplete && (
                            ...
                            >
                                <div className="text-green-700 font-semibold text-sm mb-2">✅ 설정 완료!</div>
                                        ...
                                        <CardContent>
                                            <div className="flex gap-2">
                                                {card.options.map((option) => (
                                                    <button
                                                        key={option.value}
                                                        onClick={() => handleOptionSelect(card.element, option.value)}
                                                        className={cn(
